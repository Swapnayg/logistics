...
const CitySetupList = () => {
    ...
    const [city_Province, setprovince] = useState("");
    const canSubmit = cityName.length > 0 && city_Province.length > 0;
      ...
      if($("#btncitysubmit").text().trim() == "Save")
      {
        fetch('http://localhost:5000/add_city_setup', { 
          ...
          headers: {   'Accept': 'application/json',
            ...
            body: JSON.stringify({userid:localStorage.getItem('id'), city_name: cityName, c_province: city_Province})
          }).then(res => {
              return res.json();
          }).then(data => {
          if(parseInt(data) != 0)
      ...
      else if($("#btncitysubmit").text().trim() == "Update")
        {
            fetch('http://localhost:5000/update_city_setup', { 
              ...
              headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  },
              body: JSON.stringify({userid:localStorage.getItem('id'), city_id:cityId, city_name: cityName, c_province: city_Province})
            }).then(res => {
              return res.json();
            }).then(data => {
              if(data.data == "updated")
              {
                $("#btncitysubmit").text("Save");
                refreshtable();
              }
          });
        }
  ...
  $(document).on("click", '.city_edit', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    $("#btncitysubmit").text("Update");
    var t_row =  $(this).closest('tr').index();
    var cityName = $("#city_table tbody tr:eq("+t_row+") td:eq(1)").html();
    var city_province = $("#city_table tbody tr:eq("+t_row+") td:eq(2)").attr("data-label");
    $("#edit_id").val(row_id);
    setId(row_id);
    setcityName(cityName.toString().trim());
    setprovince(city_province.toString().trim());
  });
  $(document).off('click', '.city_delete').on("click", '.city_delete', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    setId(row_id);
    ...
    <div className="col-lg-12">
      <div className="card">
        <div className="card-header">
          <h5 className="card-title mb-0">Add City Setup</h5>
        </div>
        <div className="card-body">
          <form className="row gy-3 needs-validation" noValidate onSubmit={e => e.preventDefault()}>
            ...
            <div className="col-12 mt-3 d-flex gap-2">
              <button className="btn btn-primary" id="btncitysubmit" onClick={handleSubmit} disabled={!canSubmit} >
