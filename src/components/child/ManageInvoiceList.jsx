...
const ManageInvoiceList = () => {
  const [invoiceId, setId] = useState(0);
  ...
  function refreshtable()
  {
    ...
    fetch('http://35.154.229.254/inv_invoice_data'+'/'+localStorage.getItem('id')).then((res) =>
      res.json().then((jsdata) => {
       for (let i = 0; i < jsdata.length; i++) {
        let row = '<tr>';
        row += '<td>' + (i +1) + '</td>';
        row += '<td style="text-transform:capitalize">' + jsdata[i].invoice_num + '</td>';
  ...
  const handleDownload = (e) => {
    ...
    $(document).off('click', '.invoice_view').on("click", '.invoice_view', function(e){
      e.preventDefault();
      ...
      $('#tbl_view_order tbody').empty();
        var row_id = $(this).attr("data-lable");
        fetch('http://35.154.229.254/get_inv_invoice_data', { 
          method: 'POST', 
          headers: { 'Content-Type': 'application/json', }, 
          body: JSON.stringify({userid:localStorage.getItem('id'), inv_id: row_id})
        }).then(res => {
          return res.json();
        }).then(data => {
          setv_Inv_Id(data.inv_data.id);
          setv_Inv_Num(data.inv_data.invoice_num);
          setv_Inv_Date(moment(data.inv_data.invoice_date.toString().trim()).format("DD/MM/YYYY"));
          setv_Inv_Due_Dt(moment(data.inv_data.invoice_due_date.toString().trim()).format("DD/MM/YYYY"));
          setv_Inv_Ref(data.inv_data.invoice_refer);
          if(data.inv_data.payment_terms.toString().trim()=="receipt")
          {
            setv_Inv_Term("Payment Due on Receipt");
          }
          setv_Inv_Shipp(data.inv_data.shipping);
          ...
          setv_Inv_SubTotal(parseFloat(data.inv_data.grand_total).toFixed(2) - parseFloat(data.inv_data.shipping).toFixed(2));
          setv_Inv_TotalTax(data.inv_data.total_tax);
          setv_Inv_TotalDisc(data.inv_data.total_discount);
          setv_Inv_Total(data.inv_data.grand_total);
          for (let k = 0; k < data.inv_items.length; k++) {
            var row_id = data.inv_items[k].id;
            var item_name =data.inv_items[k].item_name;
            var item_rate = data.inv_items[k].item_rate;
            var item_qty = data.inv_items[k].item_qnty;
            var item_tax = data.inv_items[k].item_tax;
            var item_disc = data.inv_items[k].item_disc_val;
            var item_amt = data.inv_items[k].item_amount;
  ...
  const handleSubmit = (e) => {
    ...
    if(canSubmit)
    {
      if($("#btnordersubmit").text().trim() == "Update")
      {
        ...
        fetch('http://35.154.229.254/update_invoice_values', { 
          method: 'POST', 
          headers: {   'Accept': 'application/json',
            'Content-Type': 'application/json'  }, 
            body: JSON.stringify({userid:localStorage.getItem('id'), o_invoiceId: invoiceId, o_invoiceNm :OInvoiceNo.toString().trim() , o_invoiceRef: ORefNo.toString().trim() , a_OInvDate: Date(OInvDate.toString().trim()), o_OInvDueDate: OInvDueDate, o_OInvTax: OInvTax.toString().trim(), o_OInvDisc: OInvDisc.toString().trim(), o_OInvNote: OInvNote.toString().trim(), o_OInvTotax: OInvTotax.toString().trim(), o_OInvDist:OInvDist.toString().trim(), o_OInvShipp:OInvShipp.toString().trim(), o_OInvGrnTotal:OInvGrnTotal.toString().trim(),OInvTotal:OInvTotal.toString().trim(), o_payment_due:payment_due.toString().trim(),o_payment_curr:payment_curr.toString().trim(), o_items : ord_items})
          }).then(res => {
              return res.json();
          }).then(data => {
          if(parseInt(data) != 0)
...
$(document).off('click', '.invoice_edit').on("click", '.invoice_edit', function(e){
  e.preventDefault();
  var row_id = $(this).attr("data-lable");
  setviewhidden(true);
  setlisthidden(true);
  setedithidden(false);
    ...
    fetch('http://35.154.229.254/get_inv_invoice_data', { 
      ...
      const due_date = new Date(data.inv_data.invoice_due_date.toString().trim());
      setId(data.inv_data.id);
      setOInvoiceNo(data.inv_data.invoice_num);
      setOInvDate(moment(data.inv_data.stock_date).format("DD/MM/YYYY"));
      setOInvDueDate(due_date);
      setpayment_curr(data.inv_data.payment_currency.toString());
      setORefNo(data.inv_data.invoice_refer.toString().trim());
      if(data.inv_data.payment_terms.toString().trim()=="receipt")
      {
        setpayment_due(data.inv_data.payment_terms.toString());
      }
      setOInvTax(data.inv_data.invoice_tax.toString());
      setOInvDisc(data.inv_data.invoice_discount.toString());
      setOInvShipp(data.inv_data.shipping);
      ...
      setOInvTotal(parseFloat(data.inv_data.grand_total).toFixed(2) - parseFloat(data.inv_data.shipping).toFixed(2));
      setOInvTotax(data.inv_data.total_tax);
      setOInvDist(data.inv_data.total_discount);
      setOInvGrnTotal(data.inv_data.grand_total);
      setOInvNote(data.inv_data.invoice_note);
      setWarehouse(data.inv_data.warehouse_id);
      setWarehouseName(data.inv_data.warehouse_name);
      var sel_Array = [];
      for (let k = 0; k < data.inv_items.length; k++) {
        var row_id = data.inv_items[k].item_product;
        sel_Array.push(parseInt(row_id)); 
        var tbl_row = $("#add_item_table tbody tr.main_row").length + 1;
        var tbl_row_indx = $("#add_item_table tbody tr").length;
        var item_name =data.inv_items[k].item_name;
        var item_rate = data.inv_items[k].item_rate;
        var item_qty = data.inv_items[k].item_qnty;
        var item_per_tax = data.inv_items[k].item_per_tax;
        var item_tax = data.inv_items[k].item_tax;
        var item_disc = data.inv_items[k].item_disc;
        var item_disc_val = data.inv_items[k].item_disc_val;
        var item_amt = data.inv_items[k].item_amount;
        var item_descp = data.inv_items[k].item_description;
  ...
  $(document).off('click', '.invoice_delete').on("click", '.invoice_delete', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    setId(row_id);
    fetch('http://35.154.229.254/invoice_inv_delete/'+row_id+'/'+localStorage.getItem('id'), { 
      method: 'DELETE', 
      headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  }, 
      body: JSON.stringify()
    ...
    <div>
       <div className="col-lg-12">
            ...
            <div className="card-header">
              <h5 className="card-title mb-0">Update Invoice</h5>
            </div>
            <div className='card-header'>
