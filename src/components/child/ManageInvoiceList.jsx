...
const ManageInvoiceList = () => {
  ...
  function refreshtable()
  {
    ...
    fetch('http://localhost:5000/inv_invoice_data'+'/'+localStorage.getItem('id')).then((res) =>
  ...
  const handleDownload = (e) => {
    ...
    $(document).off('click', '.invoice_view').on("click", '.invoice_view', function(e){
      ...
      $('#tbl_view_order tbody').empty();
        var row_id = $(this).attr("data-lable");
        fetch('http://localhost:5000/get_inv_invoice_data', { 
          method: 'POST', 
  ...
  const handleSubmit = (e) => {
    ...
    if(canSubmit)
    {
      if($("#btnordersubmit").text().trim() == "Update")
      {
        ...
        fetch('http://localhost:5000/update_invoice_values', { 
          method: 'POST', 
...
$(document).off('click', '.invoice_edit').on("click", '.invoice_edit', function(e){
  ...
  $(document).off('click', '.invoice_delete').on("click", '.invoice_delete', function(e){
    ...
    var row_id = $(this).attr("data-lable");
    setId(row_id);
    fetch('http://localhost:5000/invoice_inv_delete/'+row_id+'/'+localStorage.getItem('id'), { 
      method: 'DELETE', 
      headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  }, 
      body: JSON.stringify()
