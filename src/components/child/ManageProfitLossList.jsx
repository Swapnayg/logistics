  ...
  function refreshtable(st_date,en_date)
  {
    ...
    fetch("http://localhost:5000/get_profit_loss_details", { 
      ...
      res.json().then((jsprovdata) => {
          ...
          html += `<li class="sidebar-menu-group-title" data-label="`+jsprovdata.acct_data[i].id+`">`+jsprovdata.acct_data[i].type_name+`</li>`;
            for (let j = 0; j < jsprovdata.sub_acct.length; j++) {
              if(jsprovdata.acct_data[i].id == jsprovdata.sub_acct[j].type_name_id) {
                netprofit = (jsprovdata.acct_data[i].type_name.toString().trim() == "Incomes") ? netprofit + parseFloat(jsprovdata.sub_acct[j].sub_networth.toString().trim()) : netprofit - parseFloat(jsprovdata.sub_acct[j].sub_networth.toString().trim());
       ...
       var net_val = parseFloat(netprofit) + taxvalue;
