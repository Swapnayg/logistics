...
const ManageQuoteList = () => {


  const [quoteId, setId] = useState(0);
  const [payment_due, setpayment_due] = useState("receipt");
  const [payment_curr, setpayment_curr] = useState("Rs");
  ...
  function refreshtable()
  {
    ...
    fetch('http://localhost:5000/quote_invoice_data'+'/'+localStorage.getItem('id')).then((res) =>
      res.json().then((jsdata) => {
       for (let i = 0; i < jsdata.length; i++) {
        let row = '<tr>';
        row += '<td>' + (i +1) + '</td>';
        row += '<td style="text-transform:capitalize">' + jsdata[i].quote_num + '</td>';
        row += '<td style="text-transform:capitalize">' + jsdata[i].client_name + '</td>';
        row += '<td style="text-transform:capitalize">' + moment(jsdata[i].quote_date).format("DD/MM/YYYY") + '</td>';
        row += '<td style="text-transform:capitalize">' + "Rs. "+numberWithCommas(jsdata[i].grand_total) + '</td>';
        if(jsdata[i].invoice_status.toString().trim() == "partial")
        {
        ...
        else if(jsdata[i].invoice_status.toString().trim() == "pending")
        {
  ...
  const canSubmit =  $("#add_item_table tbody tr").length > 0 && ORefNo.length > 0;
    ...
    const handleSubmit = (e) => {
      ...
      if(canSubmit)
      {
        if($("#btnordersubmit").text().trim() == "Update")
        {
          ...
          fetch('http://localhost:5000/update_quote_invoice', { 
            method: 'POST', 
            headers: {   'Accept': 'application/json',
              'Content-Type': 'application/json'  }, 
              body: JSON.stringify({userid:localStorage.getItem('id'), o_quoteId: quoteId, o_invoiceNm :OInvoiceNo.toString().trim() , o_invoiceRef: ORefNo.toString().trim() , a_OInvDate: Date(OInvDate.toString().trim()), o_OInvDueDate: OInvDueDate, o_OInvTax: OInvTax.toString().trim(), o_OInvDisc: OInvDisc.toString().trim(), o_OInvNote: OInvNote.toString().trim(), o_OInvTotax: OInvTotax.toString().trim(), o_OInvDist:OInvDist.toString().trim(), o_OInvShipp:OInvShipp.toString().trim(), o_OInvGrnTotal:OInvGrnTotal.toString().trim(),OInvTotal:OInvTotal.toString().trim(), o_payment_due:payment_due.toString().trim(),o_payment_curr:payment_curr.toString().trim(), o_items : ord_items})
            }).then(res => {
                return res.json();
            }).then(data => {
            if(parseInt(data) != 0)
  ...
  $(document).off('click', '.quote_view').on("click", '.quote_view', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    setviewhidden(false);
    setlisthidden(true);
    ...
    $('#tbl_view_order tbody').empty();
      var row_id = $(this).attr("data-lable");
      fetch('http://localhost:5000/get_quote_invoice_data', { 
        method: 'POST', 
        ...
        setv_Inv_Id(data.quote_data.id);
        setv_Inv_Num(data.quote_data.quote_num);
        setv_Inv_Date(moment(data.quote_data.quote_date.toString().trim()).format("DD/MM/YYYY"));
        setv_Inv_Due_Dt(moment(data.quote_data.quote_due_date.toString().trim()).format("DD/MM/YYYY"));
        setv_Inv_Ref(data.quote_data.quote_refer);
        if(data.quote_data.payment_terms.toString().trim()=="receipt")
        {
          setv_Inv_Term("Payment Due on Receipt");
        }
        setv_Inv_Shipp(data.quote_data.shipping);
        setv_Inv_Cus_Name(data.client_data.client_name);
        setv_Inv_Cus_Addrs(data.client_data.client_address);
        setv_Inv_Cus_Cnty(data.client_data.client_country);
        setv_Inv_Cus_City(data.client_data.client_city);
        setv_Inv_Cus_Phone(data.client_data.client_phone.toString().trim());
        setv_Inv_Cus_Email(data.client_data.client_email);
        setv_Inv_SubTotal(parseFloat(data.quote_data.grand_total).toFixed(2) - parseFloat(data.quote_data.shipping).toFixed(2));
        setv_Inv_TotalTax(data.quote_data.total_tax);
        setv_Inv_TotalDisc(data.quote_data.total_discount);
        setv_Inv_Total(data.quote_data.grand_total);
        for (let k = 0; k < data.quote_items.length; k++) {
          var row_id = data.quote_items[k].id;
          var item_name =data.quote_items[k].item_name;
          var item_rate = data.quote_items[k].item_rate;
          var item_qty = data.quote_items[k].item_qnty;
          var item_tax = data.quote_items[k].item_tax;
          var item_disc = data.quote_items[k].item_disc_val;
          var item_amt = data.quote_items[k].item_amount;
  ...
  $(document).off('click', '.quote_edit').on("click", '.quote_edit', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    setviewhidden(true);
    setlisthidden(true);
    ...
    fetch('http://localhost:5000/get_quote_invoice_data', { 
      ...
      const due_date = new Date(data.quote_data.quote_due_date.toString().trim());
      setId(data.quote_data.id);
      setOInvoiceNo(data.quote_data.quote_num);
      setOInvDate(moment(data.quote_data.stock_date).format("DD/MM/YYYY"));
      setOInvDueDate(due_date);
      setpayment_curr(data.quote_data.payment_currency.toString());
      setORefNo(data.quote_data.quote_refer.toString().trim());
      if(data.quote_data.payment_terms.toString().trim()=="receipt")
      {
        setpayment_due(data.quote_data.payment_terms.toString());
      }
      setOInvTax(data.quote_data.quote_tax.toString());
      setOInvDisc(data.quote_data.quote_discount.toString());
      setOInvShipp(data.quote_data.shipping);
      setsupplhidden(false);
      setsupplName(data.client_data.client_name);
      setsupplAddss(data.client_data.client_address);
      setsupplCountry(data.client_data.client_country);
      setsupplCity(data.client_data.client_city);
      setsupplPhone(data.client_data.client_phone.toString().trim());
      setsupplEmail(data.client_data.client_email);
      setOInvTotal(parseFloat(data.quote_data.grand_total).toFixed(2) - parseFloat(data.quote_data.shipping).toFixed(2));
      setOInvTotax(data.quote_data.total_tax);
      setOInvDist(data.quote_data.total_discount);
      setOInvGrnTotal(data.quote_data.grand_total);
      setOInvNote(data.quote_data.quote_note);
      setWarehouse(data.quote_data.warehouse_id);
      setWarehouseName(data.quote_data.warehouse_name);
      var sel_Array = [];
      for (let k = 0; k < data.quote_items.length; k++) {
        var row_id = data.quote_items[k].item_product;
        sel_Array.push(parseInt(row_id)); 
        var tbl_row = $("#add_item_table tbody tr.main_row").length + 1;
        var tbl_row_indx = $("#add_item_table tbody tr").length;
        var item_name =data.quote_items[k].item_name;
        var item_rate = data.quote_items[k].item_rate;
        var item_qty = data.quote_items[k].item_qnty;
        var item_per_tax = data.quote_items[k].item_per_tax;
        var item_tax = data.quote_items[k].item_tax;
        var item_disc = data.quote_items[k].item_disc;
        var item_disc_val = data.quote_items[k].item_disc_val;
        var item_amt = data.quote_items[k].item_amount;
        var item_descp = data.quote_items[k].item_description;
  ...
  $(document).off('click', '.quote_delete').on("click", '.quote_delete', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    setId(row_id);
    fetch('http://localhost:5000/quote_inv_delete/'+row_id+'/'+localStorage.getItem('id'), { 
      method: 'DELETE', 
      headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  }, 
      body: JSON.stringify()
  ...
  return (
   ...
   <div className="col-lg-12">
        ...
        <div className="card-header">
          <h5 className="card-title mb-0">Update Quote</h5>
        </div>
        <div className='card-header'>
            ...
            <select 
                    id="order_pay_currency"
                    className="form-select"
                    value={payment_curr}
                    onChange={(e) => setpayment_curr(e.target.value)}
                    required> 
              ...
              <form className="row gy-3 needs-validation" noValidate onSubmit={e => e.preventDefault()}>
                <div className="col-md-5">
                 <div className="col-md-12 mb-3">
                    <h6 className="text-md fw-semibold mb-0">Bill To</h6>
                  </div>
                ...
                <div className="col-md-6">
                 <div className="col-md-12 mb-3">
                    <h6 className="text-md fw-semibold mb-0">Quote Properties</h6>
                  </div>
    ...
    <div className="card basic-data-table" hidden={listhidden}>
      <div className="card-header">
        <h5 className="card-title mb-0">Quote Invoice List</h5>
      </div>
      <div className="card-body">
        <table
...
export default ManageQuoteList;