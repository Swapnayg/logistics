...
const NewOrderList = () => {
  ...
  const loadOptions = (inputValue) =>{
    return fetch("http://localhost:5000/supplier_select",{
      method: 'POST', 
  ...
  const handleSubmit = (e) => {
    ...
    if(canSubmit)
    {
      if($("#btnordersubmit").text().trim() == "Save")
      {
        ...
        fetch('http://localhost:5000/add_purchase_order_values', { 
          method: 'POST', 
          headers: {   'Accept': 'application/json',
            'Content-Type': 'application/json'  }, 
            body: JSON.stringify({ userid:localStorage.getItem('id'), o_supplid: selectedOption.value.toString().trim(), o_warehouse: Warehouse.toString().trim(), o_invoiceNm :OInvoiceNo.toString().trim(), o_invoiceRef: ORefNo.toString().trim(), a_OInvDate: OInvDate, o_OInvDueDate: OInvDueDate, o_OInvTax: OInvTax.toString().trim(), o_OInvDisc: OInvDisc.toString().trim(), o_OInvNote: OInvNote.toString().trim(), o_OInvTotax: OInvTotax.toString().trim(), o_OInvDist:OInvDist.toString().trim(), o_OInvShipp:OInvShipp.toString().trim(), o_OInvGrnTotal:OInvGrnTotal.toString().trim(),OInvTotal:OInvTotal.toString().trim(), o_payment_due:payment_due.toString().trim(),o_update_stk:update_stk.toString().trim(), o_items : ord_items})
          }).then(res => {
              return res.json();
          }).then(data => {
          if(parseInt(data) != 0)
...
const handleDiscountChange = (e) => {
  ...
  function pad(n, length) {
   ...
   useEffect(() => {
      ...
      if (username) {
        let table;
        fetch('http://localhost:5000/get_order_invNo'+'/'+localStorage.getItem('id')).then((res) =>
          res.json().then((jsprovdata) => {
            setOInvoiceNo("ORD_" + pad(parseInt(jsprovdata.data), 4) );
          }
        )); 
