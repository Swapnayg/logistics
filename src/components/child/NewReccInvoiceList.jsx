...
const NewReccInvoiceList = () => {
  ...
  const [recc_period, setrecc_period] = useState("7 days");
  ...
  const canSubmit = selectedOption != null && $("#add_item_table tbody tr").length > 0 && Warehouse.length > 0 && ORefNo.length > 0  && ORefNo.length > 0;
  ...
  function refreshDropValues(taxvalue)
  {
    ...
    $('#add_item_table tbody tr.main_row').each(function(e) {
      ...
      $("#add_item_table tbody tr:eq("+t_row+") td:eq(5)").html(tax_val);
      $("#add_item_table tbody tr:eq("+t_row+") td:eq(7)").html(disc_val);
      $("#add_item_table tbody tr:eq("+t_row+") td:eq(8)").html(amnt_val);
  });
    ...
    if(canSubmit)
    {
      if($("#btnordersubmit").text().trim() == "Save")
      {
        ...
        fetch('http://localhost:5000/add_recc_invoice_values', { 
          ...
          headers: {   'Accept': 'application/json',
            'Content-Type': 'application/json'  }, 
            body: JSON.stringify({ userid:localStorage.getItem('id'), o_clientid: selectedOption.value, o_warehouse: Warehouse, o_invoiceNm :OInvoiceNo , o_invoiceRef: ORefNo , a_OInvDate: OInvDate, o_OInvDueDate: OInvDueDate, o_OInvTax: OInvTax, o_OInvDisc: OInvDisc, o_OInvNote: OInvNote, o_OInvTotax: OInvTotax, o_OInvDist:OInvDist, o_OInvShipp:OInvShipp, o_OInvGrnTotal:OInvGrnTotal,OInvTotal:OInvTotal, o_payment_due:payment_due,o_payment_curr:payment_curr,o_recc_period:recc_period, o_items : ord_items})
          }).then(res => {
              return res.json();
          }).then(data => {
          if(parseInt(data) != 0)
      ...
      <div className="card">
        <div className="card-header">
         ...
         <div className='card-header'>
            ...
            <label className="form-label mt-3 w-50">Period</label>
            <select 
                    id="order_pay_currency"
                    className="form-select"
                    value={recc_period}
                    onChange={(e) => setrecc_period(e.target.value)}
                    required> 
        ...
        <div className="card-body">
          <form className="row gy-3 needs-validation" noValidate onSubmit={e => e.preventDefault()}>
            ...
            <div className="col-md-6">
             <div className="col-md-12 mb-3">
                <h6 className="text-md fw-semibold mb-0">Reccuring Invoice Properties</h6>
              </div>
...
export default NewReccInvoiceList;