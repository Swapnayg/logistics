...
const OilsPartyLists = () => {
    ...
    const canSubmit = partyName.length > 0 && contactPerson.length > 0 && phoneNo.toString().length > 0;
...
const handleBack = (e) => {
  ...
  const handleSubmit = (e) => {
    ...
    if(canSubmit)
    {
      ...
      {
        fetch('http://localhost:5000/add_party', { 
          method: 'POST', 
      ...
      {
          fetch('http://localhost:5000/update_party', { 
            method: 'POST', 
            headers: { 'Content-Type': 'application/json', }, 
  ...
  {
    handleReset();
    if ( $.fn.DataTable.isDataTable('#party_table') ) {
  ...
  $(document).off('click', '.party_view').on("click", '.party_view', function(e){
    e.preventDefault();
    ...
    fetch('http://localhost:5000/ledger_account_data', { 
      ...
      headers: {   'Accept': 'application/json',
        ...
        body: JSON.stringify({userid:localStorage.getItem('id'),  ledg_id: row_id, ledg_chart_id : chart_id, ledger_type :'party'})
      }).then(res => {
          return res.json();
      }).then(data => {
  ...
  $(document).on("click", '.party_edit', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    $("#btnpartysubmit").text("Update");
    var t_row =  $(this).closest('tr').index();
    var partyname = $("#party_table tbody tr:eq("+t_row+") td:eq(1)").html();
    var party_type = $("#party_table tbody tr:eq("+t_row+") td:eq(4)").attr("data-type");
    var party_cont_per = $("#party_table tbody tr:eq("+t_row+") td:eq(2)").html();
    var party_phone = $("#party_table tbody tr:eq("+t_row+") td:eq(3)").html();
    $("#edit_id").val(row_id);
    setId(row_id);
    setPartyName(partyname.toString().trim());
    setType(party_type.toString().trim().toLowerCase());
    setContactPerson(party_cont_per.toString().trim());
    setPhoneNo(parseInt(party_phone.toString().trim()));
  });
  $(document).off('click', '.party_delete').on("click", '.party_delete', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    setId(row_id);
    fetch('http://localhost:5000/party_delete/'+row_id+'/'+localStorage.getItem('id'), { 
      method: 'DELETE', 
      headers: { 'Content-Type': 'application/json', }, 
      body: JSON.stringify({})
    }).then(res => {
