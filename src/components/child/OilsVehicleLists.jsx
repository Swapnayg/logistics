...
const OilsVehicleLists = () => {
  ...
  const canSubmit = vehicleNo.length > 0 && phoneNo.length > 0 && driverName.length > 0;
      ...
      if($("#btnvehclesubmit").text().trim() == "Save")
      {
        fetch('http://localhost:5000/add_vehicle_values', { 
          ...
          headers: {   'Accept': 'application/json',
            ...
            body: JSON.stringify({ userid:localStorage.getItem('id'),vehicle_no: vehicleNo,vehicle_type: "oil", vehicle_phone : phoneNo, vehicle_driverName : driverName})
          }).then(res => {
              return res.json();
          }).then(data => {
      ...
      else if($("#btnvehclesubmit").text().trim() == "Update")
        {
            fetch('http://localhost:5000/update_vehicle_setup', { 
              ...
              headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  },
              body: JSON.stringify({userid:localStorage.getItem('id'), veh_id:vehicleId,vehicle_type: "oil",vehicle_no: vehicleNo, vehicle_phone : phoneNo, vehicle_driverName : driverName})
            }).then(res => {
              return res.json();
            }).then(data => {
...
export default OilsVehicleLists;