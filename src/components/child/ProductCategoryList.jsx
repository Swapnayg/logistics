...
const ProductCategoryList = () => {
    ...
    const [category_id, setId] = useState(0);
    const [category_descp, setcategory_descp] = useState("");
    const canSubmit = category_name.length > 0 && category_descp.length > 0;
    var counter = 0;
    var prev_rowid = 0;
      ...
      if($("#btncategorysubmit").text().trim() == "Save")
      {
        fetch('http://localhost:5000/add_pcategory_values', { 
          ...
          headers: {   'Accept': 'application/json',
            ...
            body: JSON.stringify({userid:localStorage.getItem('id'), p_category_name: category_name, p_category_descp: category_descp})
          }).then(res => {
              return res.json();
          }).then(data => {
          if(parseInt(data) != 0)
      ...
      else if($("#btncategorysubmit").text().trim() == "Update")
        {
            fetch('http://localhost:5000/update_p_category_setup', { 
              ...
              headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  },
              body: JSON.stringify({userid:localStorage.getItem('id'), p_cat_id:category_id, p_category_name: category_name, p_category_descp: category_descp})
            }).then(res => {
              return res.json();
            }).then(data => {
  ...
  {
    handleReset();
    if ( $.fn.DataTable.isDataTable('#p_category_table') ) {
  ...
  $(document).on("click", '.p_cat_edit', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    $("#btncategorysubmit").text("Update");
    var t_row =  $(this).closest('tr').index();
    var warehouse_name = $("#p_category_table tbody tr:eq("+t_row+") td:eq(1)").html();
    var ware_code = $("#p_category_table tbody tr:eq("+t_row+") td:eq(2)").attr("data-label");
    $("#edit_id").val(row_id);
    setId(row_id);
    setcategory_name(warehouse_name.toString().trim());
    setcategory_descp(ware_code.toString().trim());
  });
  ...
  return (
    ...
    <div className="col-lg-12">
      <div className="card">
        <div className="card-header">
          <h5 className="card-title mb-0">Add Product Category</h5>
        </div>
          ...
          <form className="row gy-3 needs-validation" noValidate onSubmit={e => e.preventDefault()}>
            ...
            <div className="col-md-5">
              ...
              <textarea
                type="text"
                className="form-control"
                value={category_descp}
                onChange={(e) => setcategory_descp(e.target.value)}
                onKeyPress={(e)=>{e.target.keyCode === 13 && e.preventDefault();}}
                required></textarea>
            </div>
            ...
            <div className="col-12 mt-3 d-flex gap-2">
              <button className="btn btn-primary" id="btncategorysubmit" onClick={handleSubmit} disabled={!canSubmit} >
    ...
    <div className="card basic-data-table">
      <div className="card-header">
        <h5 className="card-title mb-0">Product Category List</h5>
      </div>
      <div className="card-body">
        <table
        ...
        >
          <thead>
            <tr>
              ...
              <th scope="col">Worth (Sales/Stock)</th>
