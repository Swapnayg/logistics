...
const SalesReportList = () => {
  ...
  const handlePrint = (e) => {
    ...
    fetch('http://35.154.229.254/generateSalesReport_pdf', { 
      method: 'POST', 
  ...
  function refreshtable(st_date,en_date)
  {
    ...
    fetch("http://35.154.229.254/get_sales_report_data", { 
      method: 'POST', 
      headers: {   'Accept': 'application/json',
        'Content-Type': 'application/json'  }, 
      ...
      res.json().then((jsdata) => {
           for (let i = 0; i < jsdata.length; i++) {
            ...
            row += '<td style="text-transform:capitalize">' + jsdata[i].rmnAmt + '</td>';
