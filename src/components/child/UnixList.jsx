...
const UnixList = () => {
    ...
    const [unitId, setId] = useState(0);
    const [unit_status, setunitstatus] = useState("");
    const canSubmit = unitName.length > 0 && unit_status.length > 0;
  ...
  const handleSubmit = (e) => {
    ...
    if(canSubmit)
    {
      ...
      {
        fetch('http://localhost:5000/add_unit_values', { 
          method: 'POST', 
          headers: {   'Accept': 'application/json',
            'Content-Type': 'application/json'  }, 
            body: JSON.stringify({ userid:localStorage.getItem('id'),unit_name: unitName, u_status: unit_status})
          }).then(res => {
              return res.json();
          }).then(data => {
          if(parseInt(data) != 0)
        ...
        {
            fetch('http://localhost:5000/update_unit_setup', { 
              method: 'POST', 
              headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  },
              body: JSON.stringify({userid:localStorage.getItem('id'), u_id:unitId,  unit_name: unitName, u_status: unit_status})
            }).then(res => {
              return res.json();
            }).then(data => {
              if(data.data == "updated")
              {
                $("#btnunitsubmit").text("Save");
                refreshtable();
              }
          });
        }
  ...
  $(document).on("click", '.unit_edit', function(e){
    ...
    $("#btnunitsubmit").text("Update");
    var t_row =  $(this).closest('tr').index();
    var unitName = $("#unit_table tbody tr:eq("+t_row+") td:eq(1)").html();
    var unit_status = $("#unit_table tbody tr:eq("+t_row+") td:eq(2)").attr("data-label");
    $("#edit_id").val(row_id);
    setId(row_id);
    setunitName(unitName.toString().trim());
    setunitstatus(unit_status.toString().trim());
  });
  $(document).off('click', '.unit_delete').on("click", '.unit_delete', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    setId(row_id);
    fetch('http://localhost:5000/unit_setup_delete/'+row_id+'/'+localStorage.getItem('id'), { 
      method: 'DELETE', 
      headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  }, 
      body: JSON.stringify({})
        ...
        <div className="card-body">
          <form className="row gy-3 needs-validation" noValidate onSubmit={e => e.preventDefault()}>
            ...
            <div className="col-12 mt-3 d-flex gap-2">
              <button className="btn btn-primary" id="btnunitsubmit" onClick={handleSubmit} disabled={!canSubmit} >
