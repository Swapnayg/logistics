...
const WarehousesList = () => {
    ...
    const [ware_id, setId] = useState(0);
    const [ware_code, setwarecode] = useState("");
    const canSubmit = warehouse_name.length > 0 && ware_code.length > 0;
    var counter = 0;
    var prev_rowid = 0;
  ...
  const handleSubmit = (e) => {
    ...
    if(canSubmit)
    {
      ...
      {
        fetch('http://localhost:5000/add_warehouse_values', { 
          method: 'POST', 
          headers: {   'Accept': 'application/json',
            'Content-Type': 'application/json'  }, 
            body: JSON.stringify({ userid:localStorage.getItem('id'),ware_stn_name: warehouse_name, ware_stn_code: ware_code})
          }).then(res => {
              return res.json();
          }).then(data => {
          if(parseInt(data) != 0)
        ...
        {
            fetch('http://localhost:5000/update_warehouse_setup', { 
              method: 'POST', 
              headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  },
              body: JSON.stringify({userid:localStorage.getItem('id'), w_id:ware_id,  ware_stn_name: warehouse_name, ware_stn_code: ware_code})
            }).then(res => {
              return res.json();
            }).then(data => {
              if(data.data == "updated")
              {
                $("#btcnwaresubmit").text("Save");
                refreshtable();
              }
          });
        }
  ...
  {
    handleReset();
    if ( $.fn.DataTable.isDataTable('#ware_table') ) {
  ...
  $(document).on("click", '.ware_edit', function(e){
    e.preventDefault();
    var row_id = $(this).attr("data-lable");
    $("#btcnwaresubmit").text("Update");
    var t_row =  $(this).closest('tr').index();
    var warehouse_name = $("#ware_table tbody tr:eq("+t_row+") td:eq(1)").html();
    var ware_code = $("#ware_table tbody tr:eq("+t_row+") td:eq(2)").html();
    $("#edit_id").val(row_id);
    setId(row_id);
    setwarehouse_name(warehouse_name.toString().trim());
    setwarecode(ware_code.toString().trim());
  });

  $(document).off('click', '.ware_delete').on("click", '.ware_delete', function(e){
      e.preventDefault();
      var row_id = $(this).attr("data-lable");
    fetch('http://localhost:5000/warehouse_delete/'+row_id+'/'+localStorage.getItem('id'), { 
      method: 'DELETE', 
      headers:{   'Accept': 'application/json',
                'Content-Type': 'application/json'  }, 
      body: JSON.stringify()
    ...
    <div className="col-lg-12">
      <div className="card">
        <div className="card-header">
          <h5 className="card-title mb-0">Add Warehouse</h5>
        </div>
        <div className="card-body">
          <form className="row gy-3 needs-validation" noValidate onSubmit={e => e.preventDefault()}>
            ...
            <div className="col-12 mt-3 d-flex gap-2">
              <button className="btn btn-primary" id="btcnwaresubmit" onClick={handleSubmit} disabled={!canSubmit} >
